...
class TensorCLIP(BaseSegmentor):
    ...
    def __init__(self,
                 ...
                 **args):
        super(TensorCLIP, self).__init__(init_cfg)
        ...
        self.core_tensor = nn.Parameter(torch.randn(self.backbone.output_dim))
        nn.init.trunc_normal_(self.contexts)
        nn.init.trunc_normal_(self.core_tensor)
    ...
    def after_extract_feat(self, x):
        ...
        score_map = torch.einsum(
            'bcc,bkc,bhwc->bkhw',
            torch.diag(self.core_tensor).expand(B, -1, -1), text, visual_embeddings.permute(0, 2, 3, 1)
        )
